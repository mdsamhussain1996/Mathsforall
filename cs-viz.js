...
(function initProbViz() {
  ...
  function draw(mu=0, sigma=1.5) {
    ...
    fillGrad.addColorStop(1, 'rgba(124,58,237,0.02)');
...
(function initOptViz() {
  ...
  function drawContours() {
    ...
    const colors = ['#00c9b8','#7c3aed','#8b5cf6','#a78bfa','#c4b5fd','#ddd6fe','#ede9fe'];
...
(function initSpanViz() {
  ...
  window.updateSpanViz = function() {
    const a1 = parseInt(document.getElementById('v1-angle').value) * Math.PI / 180;
    const a2 = parseInt(document.getElementById('v2-angle').value) * Math.PI / 180;
  ...
  function draw(v1, v2) {
    ...
    // Draw Vectors
    drawArrow(cx, cy, cx + v1.x * scale, cy - v1.y * scale, '#7c3aed', 'v₁');
    drawArrow(cx, cy, cx + v2.x * scale, cy - v2.y * scale, '#00c9b8', 'v₂');
